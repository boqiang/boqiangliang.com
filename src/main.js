...
function drawVisitorMap(countries={}){const normalized=Object.fromEntries(Object.entries(countries).map(([code,count])=>[code.toUpperCase(),Number(count)||0]));const max=Math.max(1,...Object.values(normalized));const grid=Array.from({length:7},(_,i)=>`<path class="map-grid" d="M0 ${i*16+8}H220"/>`).join('')+Array.from({length:12},(_,i)=>`<path class="map-grid" d="M${i*20-10} 0V112"/>`).join('');const shapes=worldFeatures.map(({geometry,id})=>{const code=iso3166.whereNumeric(String(id).padStart(3,'0'))?.alpha2;const count=code?normalized[code]||0:0;const ratio=count/max;const hue=count?190-(ratio*155):190;const alpha=count?.12+ratio*.58:.08;return `<path class="country-shape" d="${geoPath(geometry)}" style="--country-hue:${hue};--country-alpha:${alpha}"/>`;}).join('');worldMap.innerHTML=`<svg viewBox="0 0 220 112" role="img" aria-label="Visitor distribution map" preserveAspectRatio="xMidYMid meet"><g class="map-grid-group">${grid}</g><g class="map-land">${shapes}</g></svg>`;}
