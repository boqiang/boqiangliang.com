...
app.innerHTML = `<div class="visual-backdrop"><img class="room-photo" alt="" /><canvas id="window-renderer" aria-hidden="true"></canvas><div class="cloud-layer" aria-hidden="true"></div><div class="sun-glow" aria-hidden="true"></div></div><div class="moon-orb" aria-hidden="true"></div><canvas id="scene"></canvas><div class="visitor-modal" id="visitor-modal" aria-hidden="true"><div class="visitor-card" role="dialog" aria-modal="true" aria-labelledby="visitor-title"><button class="visitor-close" type="button" aria-label="关闭">×</button><p class="kicker">A SMALL WORLD / TODAY</p><h2 id="visitor-title">来过这里的人</h2><p class="visitor-total"><strong id="visitor-total">—</strong><span> 位访客</span></p><div class="world-map" id="world-map"></div><p class="visitor-note">点击天空或海面，查看今天的匿名访问分布。</p></div></div><div class="weather-hud" aria-hidden="true"><span class="weather-state"></span><span></span></div><button class="sound-toggle" type="button" aria-label="ambient sound" aria-pressed="false"></button>`;
...
const visitorModal=document.querySelector('#visitor-modal'), visitorTotal=document.querySelector('#visitor-total'), worldMap=document.querySelector('#world-map');
...
function drawVisitorMap(countries={}){const entries=Object.entries(countries).sort((a,b)=>b[1]-a[1]), max=Math.max(1,...entries.map(([,count])=>count)), total=entries.reduce((sum,[,count])=>sum+count,0);const markers=entries.map(([code,count])=>{const [x,y]=countryCenters[code]||countryCenters.UN;const size=Math.max(5,Math.min(18,5+count/max*13));return `<circle class="visitor-dot" cx="${x}" cy="${y}" r="${size}" style="--dot-opacity:${Math.min(.92,.42+count/max*.5)}"><title>${code}: ${count}</title></circle>`;}).join('');const ranking=entries.map(([code,count],index)=>`<div class="visitor-rank"><span>${String(index+1).padStart(2,'0')} ${code}</span><b>${count}</b><i><em style="width:${Math.round(count/Math.max(1,entries[0]?.[1]||1)*100)}%"></em></i><small>${Math.round(count/Math.max(1,total)*100)}%</small></div>`).join('');worldMap.innerHTML=`<svg viewBox="0 0 220 112" role="img" aria-label="世界地图，圆点越大表示访客越多">${worldShapes}${markers}</svg><div class="visitor-ranking"><p>访问来源排名</p>${ranking||'<span>暂时还没有统计</span>'}</div>`;}
async function refreshVisitorStats(){try{const data=await (await fetch('/api/visitors',{method:'POST'})).json();visitorTotal.textContent=data.total;drawVisitorMap(data.countries);}catch{drawVisitorMap();}}
