...
function drawVisitorMap(countries={}){const normalized=Object.fromEntries(Object.entries(countries).map(([code,count])=>[code.toUpperCase(),Number(count)||0]));const max=Math.max(1,...Object.values(normalized));const topCountries=Object.entries(normalized).filter(([,count])=>count>0).sort(([,left],[,right])=>right-left).slice(0,20);const grid=Array.from({length:7},(_,i)=>`<path class="map-grid" d="M0 ${i*16+8}H220"/>`).join('')+Array.from({length:12},(_,i)=>`<path class="map-grid" d="M${i*20-10} 0V112"/>`).join('');const mappedCountries=worldFeatures.map(({geometry,id})=>({geometry,code:iso3166.whereNumeric(String(id).padStart(3,'0'))?.alpha2}));const shapes=mappedCountries.map(({geometry,code})=>{const count=code?normalized[code]||0:0;const ratio=count/max;const hue=count?190-(ratio*155):190;const alpha=count?.12+ratio*.58:.08;return `<path class="country-shape" d="${geoPath(geometry)}" style="--country-hue:${hue};--country-alpha:${alpha}"/>`;}).join('');const countryRanks=topCountries.map(([code,count])=>{const flag=flagSvgs[code];if(!flag)return'';return `<div class="map-rank-item"><img src="data:image/svg+xml,${encodeURIComponent(flag)}" alt=""/><span>${count}</span></div>`;}).join('');const colorRanks=Array.from({length:5},(_,index)=>{const ratio=(index+1)/5;const threshold=Math.max(1,Math.ceil(max*ratio));return `<div class="map-color-item"><i style="--legend-hue:${190-(ratio*155)}"></i><span>${threshold}</span></div>`;}).join('');worldMap.innerHTML=`<div class="map-rank map-rank-left" aria-hidden="true">${countryRanks}</div><svg viewBox="0 0 220 112" role="img" aria-label="Visitor distribution map" preserveAspectRatio="xMidYMid meet"><g class="map-grid-group">${grid}</g><g class="map-land">${shapes}</g></svg><div class="map-rank map-rank-right" aria-hidden="true">${colorRanks}</div>`;}
