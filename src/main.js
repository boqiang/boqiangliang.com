...
app.innerHTML = `<div class="visual-backdrop"><img class="room-photo" alt="" /><canvas id="window-renderer" aria-hidden="true"></canvas><div class="cloud-layer" aria-hidden="true"></div><div class="sun-glow" aria-hidden="true"></div></div><div class="moon-orb" aria-hidden="true"></div><canvas id="scene"></canvas><div class="visitor-modal" id="visitor-modal" aria-hidden="true"><div class="visitor-card" role="dialog" aria-modal="true" aria-label="Visitor map"><button class="visitor-close" type="button" aria-label="关闭"></button><div class="world-map" id="world-map"></div></div></div><div class="weather-hud" aria-hidden="true"><span class="weather-state"></span><span></span></div><button class="sound-toggle" type="button" aria-label="ambient sound" aria-pressed="false"></button>`;
...
function geometryCenter(geometry){const coordinates=geometry.type==='Polygon'?geometry.coordinates:[...geometry.coordinates].flat();const points=coordinates.flat();if(!points.length)return[0,0];const center=points.reduce(([longitude,latitude],[pointLongitude,pointLatitude])=>[longitude+pointLongitude,latitude+pointLatitude],[0,0]);return projectPoint([center[0]/points.length,center[1]/points.length]);}
function drawVisitorMap(countries={}){const normalized=Object.fromEntries(Object.entries(countries).map(([code,count])=>[code.toUpperCase(),Number(count)||0]));const topCountries=Object.entries(normalized).filter(([,count])=>count>0).sort(([,left],[,right])=>right-left).slice(0,20);const grid=Array.from({length:7},(_,i)=>`<path class="map-grid" d="M0 ${i*16+8}H220"/>`).join('')+Array.from({length:12},(_,i)=>`<path class="map-grid" d="M${i*20-10} 0V112"/>`).join('');const mappedCountries=worldFeatures.map(({geometry,id})=>({geometry,code:iso3166.whereNumeric(String(id).padStart(3,'0'))?.alpha2}));const colorForCount=count=>{const bin=count<=10?0:Math.min(9,Math.floor(Math.log10(count-1)));return{hue:190-(bin/9)*155,bin};};const shapes=mappedCountries.map(({geometry,code})=>{const count=code?normalized[code]||0:0;const {hue,bin}=colorForCount(count);const alpha=count?.16+(bin/9)*.54:.08;return `<path class="country-shape" d="${geoPath(geometry)}" style="--country-hue:${hue};--country-alpha:${alpha}"/>`;}).join('');const countryRanks=topCountries.map(([code,count])=>{const flag=flagSvgs[code];if(!flag)return'';return `<div class="map-rank-item"><img src="data:image/svg+xml,${encodeURIComponent(flag)}" alt=""/><span>${count}</span></div>`;}).join('');const formatCount=count=>count.toLocaleString('en-US');const colorRanks=Array.from({length:10},(_,index)=>{const lower=index===0?0:10**index+1;const upper=10**(index+1);const {hue}=colorForCount(index===0?10:lower);return `<div class="map-color-item"><i style="--legend-hue:${hue}"></i><span>${formatCount(lower)}–${formatCount(upper)}</span></div>`;}).join('');worldMap.innerHTML=`<div class="map-rank map-rank-left" aria-hidden="true">${countryRanks}</div><svg viewBox="0 0 220 112" role="img" aria-label="Visitor distribution map" preserveAspectRatio="xMidYMid meet"><g class="map-grid-group">${grid}</g><g class="map-land">${shapes}</g></svg><div class="map-rank map-rank-right" aria-hidden="true">${colorRanks}</div>`;}
const colorRangeLabels=['≤10','>10','>100','>1K','>10K','>100K','>1M','>10M','>100M','>1B'];const renderVisitorMap=drawVisitorMap;drawVisitorMap=(countries={})=>{renderVisitorMap(countries);document.querySelectorAll('.map-color-item span').forEach((label,index)=>{label.textContent=colorRangeLabels[index]||'';});};
...
function openVisitorModal(){visitorModal.classList.add('visible');visitorModal.setAttribute('aria-hidden','false');refreshVisitorStats();}
function closeVisitorModal(){visitorModal.classList.remove('visible');visitorModal.setAttribute('aria-hidden','true');}
document.querySelector('.visual-backdrop').addEventListener('click',openVisitorModal); document.querySelector('.visitor-close').addEventListener('click',closeVisitorModal); visitorModal.addEventListener('click',e=>{if(e.target===visitorModal)closeVisitorModal();}); addEventListener('keydown',e=>{if(e.key==='Escape')closeVisitorModal();}); refreshVisitorStats();
const roomImages = {
